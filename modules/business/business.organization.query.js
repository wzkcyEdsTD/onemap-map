define("business/organization", [
    ...
    L.DCI.organization = L.Class.extend({
        ...
        _kgfind: function () {
            ...
            _map.activate(L.DCI.Map.StatusType.SELECT, this._ocallback, this.precall, this);
            ...
            var thtml = '<div class="result-business-contain">'
                ...
                + '<div class="result-contain-practice"></div>'
                + '</div>';
           ...
           // $(".result-list-group").append(this._detailsHtmlTemplet);
            $(".result-contain-practice").append(this._detailsHtmlTemplet);
        ...
        _ocallback: function (evt) {
            ...
            .run(function (error, featureCollection, response) {
                if (response.results.length > 0) {
                    ...
                    _map.activate(L.DCI.Map.StatusType.SELECT, _this._ocallback, _this.precall, _this);
                    $(".dikuai-zg").remove();
                }
            });
        },
                ...
                // a.push('084478', '084478', '084478');
                 ...
                 if (response.results.length > 0) {
                     ...
                     for (var i = 0; i < response.results.length; i++) {
                         var _geometry = JSON.stringify(response.results[i].geometry);
                         var _url = url + "3/query?geometry=" + _geometry+
                             "&geometryType=esriGeometryPolygon&inSR=&spatialRel=esriSpatialRelOverlaps&relationParam=&objectIds=&outFields=ITEMID,CASEID&where=OBJECTID+%3DOBJECTID&f=pjson";
                         _this._dciAjax.request(_url, null, null, function (data) {
                             if (data.features.length > 0) {
                                 _this._data.push(data);
                             }                            
                             var _urls = url + "4/query?geometry=" + _geometry +
                             "&geometryType=esriGeometryPolygon&inSR=&spatialRel=esriSpatialRelOverlaps&relationParam=&objectIds=&outFields=ITEMID,CASEID&where=OBJECTID+%3DOBJECTID&f=pjson";
                             _this._dciAjax.request(_urls, null, null, function (datas) {
                                 if (datas.features.length > 0) {
                                     _this._data.push(datas);
                                 }
                                 if (_this._data.length > 0) {
                                     $(".business-project-details-container").append(_this._detailshtml);
                                     for (var i = 0; i < _this._data.length; i++) {
                                         _this._data_d = [];
                                         bq._details_lenght = _this._data[i].features.length;
                                         for (var j = 0; j < _this._data[i].features.length; j++) {

                                             _this._requestProjectDetails(_this._data[i].features[j].attributes.CASEID, _this);
                                         }
                                     }
                                 }
                                 else {
                                 ...
                                 L.dci.app.util.dialog.alert("温馨提示", "获取项目详情发生错误");
                             });
                         }, function (e) {
                             L.dci.app.util.dialog.alert("温馨提示", "获取项目详情发生错误");
                         });
                        
                        // _this._requestProjectDetails(a[i], _this);
            ...
            L.esri.Tasks.identifyFeatures(url)
             ...
             .run(function (error, featureCollection, response) {
                 if (response.results.length > 0) {
                     ...
                     _map.getHighLightLayer().clearLayers();
                     L.dci.app.util.highLight(_map, feature, true, false);
            ...
            try {
                ...
                if (bq._details_num == bq._details_lenght) {
                    ...
                    for (var i = 0; i < bq._details_lenght; i++) {
                        var selector = '.proj-rarrow[value=' + bq._data_d[i].phase + ']';
                        $(selector).addClass('dq-selected shubiao-selected');
                        $(selector).bind('click', function () {
                            if (bq._data_d != null) {
                                for (var i = 0; i < bq._details_lenght; i++) {
                                    if (bq._data_d[i].phase == $(this).attr('value')) {
                                        var _selector = '.proj-rarrow[value=' + bq._data_d[i].phase + ']';
                                        // $(_selector).css('background-color', '#ffe68c');
                                        $(_selector).removeClass('dq-selected');
                                        $(_selector).addClass('selected');
                                        indicartorHtml = '';
                                        for (var j = 0; j < bq._data_d[i].indicator.comment.length; j++) {
                                            var field = bq._data_d[i].indicator.comment[j].COLUMN_NAME;
                                            var alias = bq._data_d[i].indicator.comment[j].COMMENTS;
                                            var value = bq._data_d[i].indicator.data[field];
                                            if (value != null && value.length > 0) {
                    ...
                    var selector = '.proj-rarrow[value=' + bq._data_d[0].phase + ']';
                ...
                if (bq._details_num == bq._details_lenght) {                                   //处理完所有数据再获取焦点
                    var _map = L.DCI.App.pool.get("map");
                    _map.activate(L.DCI.Map.StatusType.SELECT, bq._this_._ocallback, bq._this_.precall, bq._this_);
                }
            }
            catch (e) {
                var bq = L.DCI.App.pool.get("organizationquery");
