        ...
        */
        _GetUploadObj: function (type) {
            ...
            uploader.bind('FilesAdded', function (uploader, files) {
                if (files.length > 5) {
                    L.dci.app.util.dialog.alert("温馨提示", "至多叠加5个文件");
                    return false;
                } else {
        ...
        _GetUploadObj_new: function (num) {
            ...
            uploader.bind('FilesAdded', function (uploader, files) {
                if (files.length > uploader.settings._this.num) {
                    L.dci.app.util.dialog.alert("温馨提示", "最多叠加" + uploader.settings._this.num + "个文件");
                    return false;
                } else {
                    for (var i = 0, len = files.length; i < len; i++) {
                        var file_name = files[i].name; //文件名
        ...
        */
        ...
        addcad_ar: function (params, tUrl, callback, object) {
            $.ajax({
                ...
                error: function (er) {
                    L.dci.app.util.hideLoading();
                    L.dci.app.util.dialog.error("温馨提示", "调用CAD服务错误,请检查上传服务");
                }
            });
        },
        ...
        _fileQueueError: function (file, errorCode, message) {
            try {
                if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
                    L.dci.app.util.dialog.alert("温馨提示", "至多叠加" + this.settings.file_upload_limit + "个文件");
                    return;
                }
        ...
        */
        addshp1: function (params, tUrl) {
            $.ajax({//跨域调用
                ...
                error: function (er) {
                    L.dci.app.util.hideLoading();
                    L.dci.app.util.dialog.error("温馨提示", "调用Shapfile服务错误,请检查上传服务");
                }
            });
        },
        /**
            ...
            if (file.name.toLowerCase().indexOf("shp") != -1) {
                ...
                try {
                    ...
                    if (responseObject.status != 200) {
                        L.dci.app.util.dialog.alert("温馨提示", "上传没有成功,请重新上传");
                        return;
                    }
                } catch (ex) {
                    L.dci.app.util.hideLoading();
