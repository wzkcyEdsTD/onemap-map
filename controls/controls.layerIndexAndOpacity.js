...
define("controls/indexandopacity", [
    ...
    L.DCI.Controls.LayerIndexAndOpacity = L.Class.extend({
        ...
        */
        checkedOne: function (e) {
			var _this=this;
			 ...
			 window.liheng=$(e.target).attr("layerid");
			...
			//  $("#" + id).off("click");
			//  $("#" + id).on("click", { context: this }, function (e) {
            //    if ($("#" + id)[0].className == "reslock icon-show") {
            //        $("#" + id)[0].className = "reslock icon-hide";
            //        _this.tesFeature({ "opacity": 0, "opacityValue": "0%" },id);
            //    } else {
            //        $("#" + id)[0].className = "reslock icon-show";
            //        _this.tesFeature({ "opacity": 100, "opacityValue": "100%" },id);
            //    }
            //});
        },
		       

	   tesFeature: function (options,id) {
        ...
        //        for (var i = 0; i < this._tabNum.length; i++) {
                    ...
                    var id = id;
                    //改变tab保存的专题透明度值
                    this._tabNum[0].opacity = opacity;
                    //设置对应的专题的图层显示透明度
	   ...
	   tesOneFeature: function (options, id) {
	       var opacity = options.opacity;
	       var opacityValue = options.opacityValue;
	       var mWidth = opacity / this._scrollBar_maxValue * $("#progressBar").width();
	       var map1 = L.DCI.App.pool.get("map").map;
        ...
        */
        ...
        changeLayerVisibleStatus: function (id) {
            if ($("#" + id)[0].className == "reslock visibleIcon icon-show") {
                $("#" + id)[0].className = "reslock visibleIcon icon-hide";
                this.tesOneFeature({ "opacity": 0, "opacityValue": "0%" },id);
            } else {
                $("#" + id)[0].className = "reslock visibleIcon icon-show";
                this.tesOneFeature({ "opacity": 100, "opacityValue": "100%" }, id);
            }
        },
        /**
        *添加一个专题图层
        ...
        */
        addFeature: function (id, name) {
            ...
            var html = '<div class="featureProgressBarContainer">'
                            ...
                            + '</div>'
						//	+ '<span class="reslock ' + lockedClass + '" lock="' + locked + '" style="float:right;font-size: 22px;color: #ffb400;"></span>'
							+ '<span class="reslock visibleIcon icon-show" id="' + id + '"  style="float:right;font-size: 22px;color: #ffb400;"></span>'
                            + '<div class="featureContent" style="width:180px">'
                                + '<div id="progressBar">'
            ...
            $(".visibleIcon").click(function () {
                _this.changeLayerVisibleStatus(this.id);
            });
        },
        ...
        */
        removeFeature: function (options) {
			// if (Project_ParamConfig.prolayname.includes(options.name)) {
            //    if(window.fename.includes(options.name)){
            //    window.fename.splice($.inArray(options.name, window.fename), 1);
            //    if (window.fename.length == 0) {
            //        $(".pro-lay").hide();
            //      }
            //   }
            //}
        ...
        */
        deleteBtn: function () {
            ...
            while(this._tabNum.length!=0){
				...
				//if (Project_ParamConfig.prolayname.includes(name)) {
                //     if (window.fename.includes(name)) {
                //         window.fename.splice($.inArray(name, window.fename), 1);
                //        if (window.fename.length == 0) {
                //            $(".pro-lay").hide();
                //        }
                //    }
        ...
        setFeature: function (options) {
            ...
            if (this._tabNum.length > 0) {
                ...
                for (var i = 0; i < this._tabNum.length; i++) {
                    //获取专题id
                    var id = this._tabNum[i].id;
                    //若是已经隐藏，则拖动滚动条不改变透明度 bychenx
                    if ($("#" + id)[0].className == "reslock visibleIcon icon-hide") {
                        //添加一个提示文字
                        if ($("#wholeliftcycle-Tip").length == 0) {
                            var html = '<div id="wholeliftcycle-Tip">该图层已隐藏，无法更改透明度！</div>';
                            $(".out-container").append(html);
                        } else {
                            $("#wholeliftcycle-Tip").text("该图层已隐藏，无法更改透明度！");
                        }
