...
define("controls/groupedlayer", [
    ...
    L.DCI.Controls.GroupedLayers = L.Control.extend({
        ...
        _initLayout: function () {
            ...
            container.mapId = this._map.options.id;
          
            L.DomUtil.addClass(this._container, 'baselayer-control');
            container.id =  this._map.options.id+'-baselayer-control';
            
            container.setAttribute('aria-haspopup', true);

            var form = this._form = L.DomUtil.create('form', className + '-list');
            form.id = this._map.options.id + '-' + className + '-list';

            if (this.options.collapsed) {               
                var link = this._layersLink = L.DomUtil.create('a', className + '-toggle', container);
                link.href = '#';
                link.title = '背景底图';
                link.id = this._map.options.id + "-layerSwitch";
        ...
        _update: function () {
            ...
            for (i in this._layers) {
                obj = this._layers[i];
                this._addItem(obj, this._container.mapId);
        ...
        _addItem: function (obj, mapId) {
            ...
            if (obj.overlay) {
                ...
                label.id = mapId+"$leaflettile=" + obj.layer.options.id;
                ...
                subLabel.id = mapId+"$leaflettilesub=" + obj.layer.options.id;
                subLabel.layerId = L.Util.stamp(obj.layer);
                subLabel.layerStatus = false;
                var span = document.createElement("span");
                span.innerHTML = obj.name;
                span.id = mapId+"$leaflettilespan=" + obj.layer.options.id;
        ...
        */
        _expandOrCollapse: function (e) {
            ...
            var mapId = id.split('-')[0];
            //判断是否打开了时间轴，若打开则关闭。二者为互斥事件
            var timeEle = $("#" + mapId + "-timeslider-control");
        ...
        _changeBaseLayer: function (e) {
            ...
            var mapId = e.target.id.split('$')[0];
            var con = document.getElementById(mapId+"$leaflettilesub=" + id);
            ...
            if (con.layerStatus) {
                ...
                var child = $("#" + mapId + "-leaflet-control-layers-list").find(".leaflet-control-layers-group-tile").children();
                ...
                $("#" + mapId + "-layerSwitch").css("background-image", updateImg);
            }
        }
    });
