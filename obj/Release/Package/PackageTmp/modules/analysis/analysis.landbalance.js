...
define("analysis/landbalance", [
    ...
    L.DCI.LandBalance = L.Class.extend({
        /**
        *类ID
        *@property id
        *@type {String}
        ...
        */
        initialize: function () {
            ...
            $(".leftcontentpanel-title>span:first").html("用地平衡分析");         //标题
            ...
            //输出表格
            $(".ydph-table-wrap .print").on('click', { context: this }, function (e) { e.data.context.printExcel(e); });
            ...
            $("#toxls").on('click', { context: this }, function (e) {
                ...
                L.txls.converttoxls(".ydph-table-wrap");
                //_this.saveCode();



                /*
                var table = '<div class="temptable"></div>';
                $(".leftcontentpanel-body").append(table);
                $(".temptable").append($(".ydph-table-wrap").html());

                var trs = $(".temptable thead tr");
                var tablehead = "";
                for (var i = 0; i < trs.length; i++) {
                    ...
                    for (var j = 0; j < ths.length; j++) {
                        if (i == 0) {
                            if ($(ths[j]).hasClass("colspan45") || $(ths[j]).hasClass("colspan67") || $(ths[j]).hasClass("colspan89")) {
                                temptrs += '<td colspan="2">' + ths[j].innerHTML + '</td>';
                            } else {
                                temptrs += '<td rowspan="2">' + ths[j].innerHTML + '</td>';
                            }
                        } else {
                            temptrs += '<td>' + ths[j].innerHTML + '</td>';
                        }
                    }
                    
                    tablehead += "<tr>" + temptrs + "</tr>";
                }
                $(".temptable thead tr").remove();
                $(".temptable tbody tr:first").before(tablehead);

                $(".temptable .table tr").css({ "display": "block" });
                $(".temptable .table").tableExport({ type: 'excel', escape: 'false' });
                $(".temptable").remove();
                */
            });
        },
        ...
        saveCode:function (obj) {
            ...
            var strHTML = $(".ydph-talbe-head").html();
        ...
        */
        addService:function(){
            ...
            L.esri.dynamicMapLayer(this.url, { id: '用地平衡分析', layers: [this.layerIndex] }).addTo(map);
        },
       /**
       *删除专题服务
       ...
       */
        deleteService:function(){
            ...
            _map.eachLayer(function (layer) {
                if (layer.options && layer.options.id == "用地平衡分析" && layer.options.id != null)
                {
        ...
        setTabsHTML: function () {
            ...
            $(".acharts-charts-tool").on('click', function () {
                if ($(".acharts-charts-tool").hasClass("icon-Viewform")) {
                    ...
                    $(".ydph-table-wrap").css({ "display": "block" });
                } else {
                    ...
                    $(".ydph-table-wrap").css({ "display": "none" });
       ...
       */
        getTabsData1: function () {
            ...
            L.dci.app.services.baseService.getXingzheng({
                ...
                success: function (res) {
                    this.tabsData1 = null;
                    this.tabsData1 = {
                        "district": Project_ParamConfig.ydphfxConfig.xzqh,
                        "towns": [
                        ]
                    };
                    for (var rn in res) {
        ...
        */
        showContent: function () {
            ...
            //展开或隐藏table
            $(".ydph-talbe-body").on('click', '.hastables', { context: this }, function (e) { e.data.context.showTrEvent(e); });
            $(".ydph-talbe-body").on('click', '.colspan3', { context: this }, function (e) { e.data.context.clickTrEvent(e); });
            //点击表头，回复显示所有类型地块
            $(".ydph-talbe-head .colspan3").on('click', { context: this }, function (e) { e.data.context.landType(e); });
        },
        /**
        *插入table整体布局
        *@method setTableHTML
        ...
        */
        setTableHTML:function(){
            var html = '<div class="ydph-talbe-head">'
                        + '<table class="table table-bordered" id="ydphTable1">'
                            + '<thead>'
                                + '<tr>'
                                    + '<th rowspan="2" class="colspan1"></th>'
                                    + '<th rowspan="2" class="colspan2">用地代码</th>'
                                    + '<th rowspan="2" class="colspan3">用地名称</th>'
                                    + '<th colspan="2" class="colspan45">用地总面积(m²)</th>'
                                    + '<th colspan="2" class="colspan67">面积比例(%)</th>'
                                    + '<th colspan="2" class="colspan89">人均用地(m²)</th>'
                                + '</tr>'
                                + '<tr>'
                                    + '<th class="colspan4">规划</th>'
                                    + '<th class="colspan5">现状</th>'
                                    + '<th class="colspan6">规划</th>'
                                    + '<th class="colspan7">现状</th>'
                                    + '<th class="colspan8">规划</th>'
                                    + '<th class="colspan9">现状</th>'
                                + '</tr>'
                            + '</thead>'
                        + '</table>'
                    + '</div>'
                    +'<div class="ydph-talbe-body">'
                        + '<table class="table table-bordered firsttable" id="ydphTable">'
                            + '<tbody class="ydph-talbe-tbody">'
                            + '</tbody>'
                        +'</table>'
                      + '</div>'
            ...
            $(".ydph-table-wrap").html(html);

            //滚动条
            $(".ydph-talbe-body").mCustomScrollbar({
                theme: "minimal-dark"
            });
        },
        /**
        ...
        */
        setTable:function(data){
            var container = ".ydph-talbe-tbody";
            $(container).html("");
            //计算城市用地总面积
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                {//普通的一级tr
                    ...
                    tr = '<tr class="levelOne" level="1" dataid="' + data[i].MARK + '" pid="" gpid="">'
                        ...
                        + '<td class="colspan5"></td>'
                        + '<td class="colspan6">' + (data[i].area / _totalArea_plan).toFixed(2) + '</td>'
                        + '<td class="colspan7"></td>'
                        + '<td class="colspan8">' + data[i].popuarea + '</td>'
                ...
                {//含二级元素的一级tr
                    ...
                    {
                        var secondTr = '';
                        if (secondData[j].items.length == 0)
                        {//普通的二级tr
                            ...
                            secondTr = '<tr class="levelTwo" level="2" dataid="' + secondData[j].MARK + '" pid="' + data[i].MARK + '" gpid="' + data[i].MARK + '" style="display:none;">'
                                        ...
                                        + '<td class="colspan5"></td>'
                                        + '<td class="colspan6">' + (secondData[j].area / _totalArea_plan).toFixed(2) + '</td>'
                                        + '<td class="colspan7"></td>'
                                        + '<td class="colspan8">' + secondData[j].popuarea + '</td>'
                                        + '<td class="colspan9"></td>'
                                     + '</tr>';
                        ...
                        {//含三级元素的二级tr
                            ...
                            for (var k = 0; k < thirdData.length; k++)
                            {
                                ...
                                thirdTr = '<tr class="levelThree" level="3" dataid="' + thirdData[k].MARK + '" pid="' + secondData[j].MARK + '" gpid="' + data[i].MARK + '" style="display:none;">'
                                            ...
                                            + '<td class="colspan5"></td>'
                                            + '<td class="colspan6">' + (thirdData[k].area / _totalArea_plan).toFixed(2) + '</td>'
                                            + '<td class="colspan7"></td>'
                                            + '<td class="colspan8">' + thirdData[k].popuarea + '</td>'
                                            + '<td class="colspan9"></td>'
                                        + '</tr>';
        ...
        */
        insertTrTotal: function () {
           ...
           var tr = '<tr>'
                        ...
                        + '<td class="colspan7"></td>'
                        + '<td class="colspan8">' + perCapita_plan + '</td>'
                        + '<td class="colspan9"></td>'
                      + '</tr>';
           var container = ".ydph-talbe-tbody";
           $(container).append(tr);
        ...
        */
        clickLiEvent: function (e) {
            ...
            if (bool != -1) {                
                ...
                var text = $(e.target).text();
                $(".ydph-nav-btn-wrap button")[0].innerHTML = text;
            }
            ...
            //查询行政边界
            var currentData = [];
            var url = Project_ParamConfig.ydphfxConfig.xingzheng_layer.url;
            var id = Project_ParamConfig.ydphfxConfig.xingzheng_layer.field;
            var layerIndex = Project_ParamConfig.ydphfxConfig.xingzheng_layer.layerIndex;

            var query = new L.esri.Tasks.find(url);
            query.params.sr = Project_ParamConfig.ydphfxConfig.xingzheng_layer.sr;
                ...
                if (featureCollection != null) {
                    ...
                    if (Project_ParamConfig.ydphfxConfig.PreResult) {
                        ...
                        this.ajax.get(newUrl, null, true, this, function(res) {
                            ...
                            Project_ParamConfig.ydphfxConfig.inputParams.In_region = this.in_region;
                            this.getService();
                        });
                    } else {
        ...
        */
        showTrEvent: function (e) {
            ...
            if ($(e.target).parent('.hastables').hasClass(tr_active))
            {
                ...
                {
                    //隐藏三级元素
                    $(".ydph-talbe-tbody>tr[pid='" + id + "']").css("display", "none");

                }
                else
                {
                    //隐藏二级和三级元素
                    $(".ydph-talbe-tbody>tr.hastables[pid='" + id + "']").removeClass(tr_active);
                    $(".ydph-talbe-tbody>tr.hastables[pid='" + id + "']").children().first().text("+");
                    $(".ydph-talbe-tbody>tr.hastables[pid='" + id + "']").children().first().removeClass(sign_active);
                    $(".ydph-talbe-tbody>tr[gpid='" + id + "']").css("display", "none");
                }
            }
            else
            {
                ...
                $(".ydph-talbe-tbody>tr[pid='" + pid + "']").css("display", "table-row");       //显示下级元素
            }
        },
        //输出表格
        ...
        */
        clickTrEvent: function (e) {
            ...
            if (!$(e.target).hasClass("selected")) {
                var aEle = $(".ydph-talbe-body").find(".colspan3");
                for (var i = 0; i < aEle.length; i++) {
            ...
            if (obj.hasClass("hastables"))
            {
                ...
                switch (level) {
                    case "1":
                        ...
                        var ele = $(".ydph-talbe-tbody>tr[gpid='" + id + "']");
                    ...
                    case "2":
                        ...
                        var ele = $(".ydph-talbe-tbody>tr[pid='" + id + "']");
                        for (var i = 0; i < ele.length; i++) {
                            var childrenId = $(ele[i]).attr("dataid");
                            array.push(childrenId);
                        }
        ...
        */
        DefLayer: function (array, index) {
            ...
            _map.eachLayer(function (layer) {
                if (layer.options && layer.options.id != "baseLayer" && layer.options.id == "用地平衡分析") {
                    //设置图层透明度
        ...
        */
        getService: function () {
            ...
            gp.GPHandler(Project_ParamConfig.ydphfxConfig.url,
                Project_ParamConfig.ydphfxConfig.inputParams,
                Project_ParamConfig.ydphfxConfig.outParams,
